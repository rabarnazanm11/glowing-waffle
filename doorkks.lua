...
local SHOW = {
	DoorHighlight     = false,
	DoorLabel         = true,   -- ← NEW: BillboardGui labels for doors
	DoorTracer        = true,
	ItemHighlight     = true,
	ItemTracer        = true,
...
-- ============================================================
-- GET DOOR PART FOR HIGHLIGHT/LABEL
-- ============================================================
local function getDoorTarget(room)
	local doorModel = room:FindFirstChild("Door")
	if doorModel then
		local doorPart = doorModel:FindFirstChild("Door") -- workspace.CurrentRooms["0"].Door.Door
...
if not doorActive or not curRoom then
	...
	for _, d in ipairs(cachedDoors) do
		if d.highlightInst then removeHL(doorHighlights, d.highlightInst) end
		if d.labelInst then removeDoorBillboard(d.labelInst) end
	end
	cachedDoors = {}
else
	local roomSet = {}
	...
	for roomNum in pairs(roomSet) do
		...
		if room then
			local targetPart, labelParent = getDoorTarget(room)
			if targetPart then
				local pos = getPos(targetPart)
				if pos then
					...
					table.insert(newCache, { 
						...
						roomNum=roomNum, 
						locked=locked,
						highlightInst=targetPart,      -- Highlight the actual Door.Door part
						labelInst=labelParent or targetPart, -- Billboard attaches to this
						labelText=labelText,
						labelColor=color
					})
					...
					if SHOW.DoorHighlight then
						applyHL(doorHighlights, targetPart, color)
					end
					-- Create/Update BillboardGui label
					if SHOW.DoorLabel then
						createDoorBillboard(labelParent or targetPart, labelText, color)
					end
				end
			end
		end
...
RunService.Heartbeat:Connect(function()
    ...
    -- Door ESP (BillboardGui handles labels, only draw tracers)
for _, d in ipairs(cachedDoors) do
	-- Update Billboard text with fresh distance (since player moves)
	if SHOW.DoorLabel and d.labelInst then
		...
		updateDoorBillboard(d.labelInst, newText, d.labelColor)
	end
	
	-- Draw tracer using original Drawing method (unchanged)
	if SHOW.DoorTracer then
		local sPos, onScreen, depth = W2S(d.pos)
		if onScreen and depth > 0 then
			makeTracer(sPos, d.labelColor)
		end
	end
...
SettingsTab:AddToggle({ 
	...
	Callback = function(value) 
		...
		if not value then 
			...
			for _, d in ipairs(cachedDoors) do 
				if d.labelInst then 
					createDoorBillboard(d.labelInst, d.labelText, d.labelColor) 
				end 
			end 
		end
	end 
