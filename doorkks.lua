-- ============================================================
-- DOORS HUB  (Updated with BillboardGui Doors)
-- ============================================================
...
local ENTITIES = {
    ...
    { Name = "FigureRig",       Display = "Figure",        Color = Color3.fromRGB(80,  80,  80),  notify = true,  highlight = true,  tracer = true  },    { Name = "Snare",           Display = "Snare",         Color = Color3.fromRGB(80,  80,  80),  notify = false, highlight = true,  tracer = false },
...
local CUSTOM_ESP = {
    ...
    { Name = "Toolshed_Small", Display = "Toolshed_Small",  Color = Color3.fromRGB(80,  120, 200), toggle = false, highlights = {}, cached = {} },    { Name = "WaterPump",      Display = "WaterPump",       Color = Color3.fromRGB(80,  120, 200), toggle = false, highlights = {}, cached = {} },
}
...
local CharCrouchOn   = truelocal CharSpeedValue = 4
...
local function getPos(inst)
    ...
    if bp then return bp.Position end    local ok, piv = pcall(function() return inst:GetPivot() end)
...
local function enableSeekGuidePath()
    for _, inst in ipairs(Workspace:GetDescendants()) do
        if inst:IsA("BasePart") and inst.Name:match("^MinecartNode") then
            pcall(function()
                inst.Transparency = 0                seekGuideModified[inst] = true
            end)
        end
    end
...
task.spawn(function()
    while true do
        ...
        if not WardrobeESPToggle then
            ...
            local newCache, seen = {}, {}
            for _, inst in ipairs(CurrentRooms:GetDescendants()) do
                if (inst.Name == "Wardrobe" or inst.Name == "Toolshed") and isInESPRange(inst) then                    local pos = getPos(inst)
                    if pos then
        ...
        -- Doors [PATCHED: BillboardGui Version]
        local doorActive = DoorRangeToggle or DoorNextToggle
        if not doorActive or not curRoom then
            -- Cleanup all door highlights and billboards
            for _, d in ipairs(cachedDoors) do
                if d.highlightInst and doorHighlights[d.highlightInst] then 
                    doorHighlights[d.highlightInst]:Destroy()
                    doorHighlights[d.highlightInst] = nil 
                end
                if d.labelInst and DoorESPPatch then DoorESPPatch.removeBillboard(d.labelInst) end
            end
            cachedDoors = {}
        else
            local roomSet = {}
            ...
            local newCache, seen = {}, {}
            for roomNum in pairs(roomSet) do                local room = CurrentRooms:FindFirstChild(tostring(roomNum))
                if room then
                    local doorModel = room:FindFirstChild("Door")
                    local targetPart = nil
                    local labelParent = nil
                    
                    if doorModel then
                        targetPart = doorModel:FindFirstChild("Door")
                        if not targetPart or not targetPart:IsA("BasePart") then
                            targetPart = doorModel:FindFirstChildWhichIsA("BasePart", true)
                        end
                        labelParent = doorModel
                    end
                    if not targetPart then
                        targetPart = room:FindFirstChild("RoomEntrance")
                        labelParent = targetPart
                    end
                    ...
                    if targetPart then
                        ...
                        if pos then
                            local locked = false
                            if doorModel then
                                locked = doorModel:FindFirstChild("Lock") ~= nil
                            end
                            -- 🔴 Red for locked, 🟢 Green for unlocked
                            local color = locked and Color3.fromRGB(255, 60, 60) or Color3.fromRGB(60, 255, 60)
                            local isNext = (roomNum == curRoom + 1)
                            local prefix = isNext and "NEXT [" or "Entrance ["
                            local lockIcon = locked and "🔒 " or ""
                            local dist = math.floor((pos - HRP.Position).Magnitude)
                            local labelText = lockIcon .. prefix .. roomNum .. "] | " .. dist .. "M"
                            ...
                            table.insert(newCache, { 
                                ...
                                roomNum=roomNum, 
                                locked=locked,
                                highlightInst=targetPart,
                                labelInst=labelParent or targetPart,
                                labelText=labelText,
                                labelColor=color
                            })
                            seen[targetPart] = true
                            
                            -- Apply Highlight to the actual door part
                            if SHOW.DoorHighlight then
                                if not doorHighlights[targetPart] then
                                    local h = Instance.new("Highlight")
                                    ...
                                    h.DepthMode = Enum.HighlightDepthMode.AlwaysOnTop
                                    h.Parent = targetPart
                                    doorHighlights[targetPart] = h
                                else
                                    doorHighlights[targetPart].FillColor = color
                                    doorHighlights[targetPart].OutlineColor = color
                                end
                            end
                            -- Create/Update BillboardGui label
                            if SHOW.DoorLabel and DoorESPPatch then
                                DoorESPPatch.createBillboard(labelParent or targetPart, labelText, color)
                            end
                        end
                    end
                end
            end
            -- Cleanup old entries that are no longer in range
            for _, d in ipairs(cachedDoors) do
                if not seen[d.inst] then
                    if d.highlightInst and doorHighlights[d.highlightInst] then 
                        doorHighlights[d.highlightInst]:Destroy()
                        doorHighlights[d.highlightInst] = nil 
                    end
    ...
    while true do
                ...
                if (n == "ChestBox" or n == "ChestBoxLocked" or n == "Chest_Vine") and isInESPRange(inst) then
                    ...
                    if pos then
                        local col = n == "ChestBox" and Color3.fromRGB(180, 0, 255)
                               or  n == "ChestBoxLocked" and Color3.fromRGB(255, 100, 200)                               or  Color3.fromRGB(80, 200, 80)
        ...
        for _, entry in ipairs(CUSTOM_ESP) do
            if not entry.toggle then
                ...
                local new, seen = {}, {}
                for _, inst in ipairs(CurrentRooms:GetDescendants()) do                    if inst.Name == entry.Name and isInESPRange(inst) then
                        local pos = getPos(inst)
                        if pos then
                            table.insert(new, { inst=inst, pos=pos }); seen[inst] = true
...
task.spawn(function()
    while true do
        ...
        pcall(function()
            ...
            if CharCrouchOn then char:SetAttribute("Crouching", true) end            char:SetAttribute("SpeedBoost", CharSpeedValue)
        end)
    end
end)
...
RunService.Heartbeat:Connect(function()
    ...
    -- Door ESP [PATCHED: BillboardGui handles labels, tracers unchanged]
    for _, d in ipairs(cachedDoors) do
        -- Update Billboard text with fresh distance (player moves)
        if SHOW.DoorLabel and d.labelInst and DoorESPPatch then
            local dist = math.floor((d.pos - HRP.Position).Magnitude)
            local isNext = (d.roomNum == LocalPlayer:GetAttribute("CurrentRoom") + 1)
            local prefix = isNext and "NEXT [" or "Entrance ["
            local lockIcon = d.locked and "🔒 " or ""
            local newText = lockIcon .. prefix .. d.roomNum .. "] | " .. dist .. "M"
            DoorESPPatch.updateBillboard(d.labelInst, newText, d.labelColor)
        end
        
        -- Draw tracer using ORIGINAL Drawing method (unchanged as requested)
        if SHOW.DoorTracer then
            local sPos, onScreen, depth = Camera:WorldToViewportPoint(d.pos)
            if onScreen and depth > 0 then
                local tracer = Drawing.new("Line")
                tracer.From = Vector2.new(Camera.ViewportSize.X / 2, Camera.ViewportSize.Y)
                tracer.To = Vector2.new(sPos.X, sPos.Y)
                tracer.Color = d.labelColor
                tracer.Thickness = CFG.TracerThick
                tracer.Transparency = CFG.TracerAlpha
                tracer.Visible = true
                table.insert(espDrawings, tracer)
            end
        end
        -- ✅ Label is now handled by BillboardGui - no Drawing.new("Text") needed
    end
...
PlayerTab:AddToggle({
    ...
    Callback = function(value)
        ...
        if value then
            ...
            pcall(function()                local hrp = Character:FindFirstChild("HumanoidRootPart") or Character.PrimaryPart
                if hrp then
        ...
        if value then
            if not autoProxiConn then
                autoProxiConn = RunService.Heartbeat:Connect(function()
                    ...
                    for _, v in ipairs(workspace:GetDescendants()) do                        if v:IsA("ProximityPrompt") and v.Enabled then
                            local blocked = false
                            local p = v.Parent
...
ESPTab:AddToggle({
    Name = "Entrance ESP (±5 rooms)", Description = "ESP room entrances — green=unlocked, red=locked",
    Default = true,
    Callback = function(value) DoorRangeToggle = value end
})

ESPTab:AddToggle({
    Name = "Next Room Entrance", Description = "ESP only the very next room entrance",
    Default = false,
...
ESPTab:AddToggle({    Name = "Item ESP", Description = "ESP items in current room ±1",
    Default = true,
    Callback = function(value)
        ItemESPToggle = value
...
local bridgeFixParts = {}local bridgeFixConn = nil
...
local function cleanupGuideBall()    if guideBall and guideBall.Parent then
        guideBall:Destroy()
    end
    guideBall = nil
    currentWaypoints = {}
...
local function computePathToExit()
    ...
    if curRoomNumRaw then
        ...
        elseif type(curRoomNumRaw) == "string" then
            curRoomNum = tonumber(curRoomNumRaw:match("^%d+"))        end
    end
...
VisualTab:AddToggle({
    ...
    Callback = function(value)
        ...
        if value then
            ...
            if not guideBallConn then                guideBallConn = RunService.Heartbeat:Connect(function()
...
SettingsTab:AddToggle({
    ...
    Callback = function(value)
        ...
        if not value then clearHL(doorHighlights)
            ...
            for _, d in ipairs(cachedDoors) do 
                local color = d.locked and Color3.fromRGB(255, 60, 60) or Color3.fromRGB(60, 255, 60)
                if d.highlightInst then
                    if doorHighlights[d.highlightInst] then
                        doorHighlights[d.highlightInst].FillColor = color
                        doorHighlights[d.highlightInst].OutlineColor = color
                    else
                        local h = Instance.new("Highlight")
                        h.FillColor = color
                        h.OutlineColor = color
                        ...
                        h.OutlineTransparency = 0
                        h.DepthMode = Enum.HighlightDepthMode.AlwaysOnTop
                        h.Parent = d.highlightInst
                        doorHighlights[d.highlightInst] = h
                    end
                end
            end         end
    end
...
SettingsTab:AddToggle({ 
    Name = "Door Label (BillboardGui)", 
    Description = "Show door names/distance as 3D BillboardGui labels",
    Default = true, 
    Callback = function(value) 
        SHOW.DoorLabel = value
        if not value then 
            for _, d in ipairs(cachedDoors) do 
                if d.labelInst and DoorESPPatch then DoorESPPatch.removeBillboard(d.labelInst) end 
            end 
            ...
            for _, d in ipairs(cachedDoors) do 
                if d.labelInst and DoorESPPatch then 
                    DoorESPPatch.createBillboard(d.labelInst, d.labelText, d.labelColor) 
                end 
            end 
        end
    end 
...
task.defer(function()
    ...
    if not InstantInteractConn then        InstantInteractConn = PPS.PromptButtonHoldBegan:Connect(function(prompt)
            pcall(function() fireproximityprompt(prompt) end)
        end)
    end
...
-- ============================================================
-- ★ DOOR ESP PATCH - BILLBOARDGUI VERSION ★
-- ============================================================

-- Safe wrapper to prevent errors
local function SafeDoorESP_Patch()
    -- Check if required services exist
    local WS = game:GetService("Workspace")
    local CR = WS:FindFirstChild("CurrentRooms")
    if not CR then 
        warn("DoorESP Patch: CurrentRooms not found")
        return 
    end
        -- Helper: Create BillboardGui for door
    local function createDoorBillboard(inst, text, color)
        if not inst then return end
        
        -- Remove existing
        local existing = inst:FindFirstChild("DoorBillboard")
        if existing then existing:Destroy() end
        
        local success, bb = pcall(function()
        ...
        if not success then
            warn("DoorESP Patch: Failed to create billboard -", tostring(bb))
        end
    end
    
    -- Helper: Update existing billboard
    local function updateDoorBillboard(inst, text, color)
        if not inst then return end
        local bb = inst:FindFirstChild("DoorBillboard")
        if bb and bb:FindFirstChild("TextLabel") then
            local tl = bb.TextLabel
            if text then tl.Text = text end
            if color then tl.TextColor3 = color end        end
    end
    
    -- Helper: Remove billboard
    local function removeDoorBillboard(inst)
    ...
    -- Helper: Get door target part
    local function getDoorTarget(room)
        if not room then return nil end
        local doorModel = room:FindFirstChild("Door")
        if doorModel then
            local doorPart = doorModel:FindFirstChild("Door")
            if doorPart and doorPart:IsA("BasePart") then
                return doorPart, doorModel
            end
            local bp = doorModel:FindFirstChildWhichIsA("BasePart", true)
            if bp then return bp, doorModel end
        end
        return room:FindFirstChild("RoomEntrance"), nil
    end
    
    -- Helper: Check if door is locked
    local function isDoorLocked(room)
    ...
    print("✅ DoorESP BillboardGui Patch Loaded Successfully!")
    -- Return functions for use in main script
    return {
        createBillboard = createDoorBillboard,
        updateBillboard = updateDoorBillboard,
        removeBillboard = removeDoorBillboard,
        getDoorTarget = getDoorTarget,
        isDoorLocked = isDoorLocked
    }
end

-- Try to load the patch
local DoorESPPatch = SafeDoorESP_Patch()

-- If patch loaded successfully, you can now use it
if DoorESPPatch then
    print("🚪 Door ESP Patch is ready to use!")    print("   - BillboardGui labels: GREEN = unlocked, RED = locked")
    print("   - Highlights target: workspace.CurrentRooms[X].Door.Door")
    end
