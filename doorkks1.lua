...
if not doorActive or not curRoom then
	...
	for _, d in ipairs(cachedDoors) do
		...
		if d.labelInst then DoorESPPatch.removeBillboard(d.labelInst) end
	end
	cachedDoors = {}
else
	local roomSet = {}
	...
	for roomNum in pairs(roomSet) do
		...
		if room then
			local targetPart, labelParent = DoorESPPatch.getDoorTarget(room)
			if targetPart then
				local pos = targetPart.Position
				if pos then
					local locked = DoorESPPatch.isDoorLocked(room)
...
for _, d in ipairs(cachedDoors) do
	...
	if SHOW.DoorLabel and d.labelInst then
...
SettingsTab:AddToggle({
    ...
    Callback = function(value)
        ...
        else for _, d in ipairs(cachedDoors) do applyHL(doorHighlights, d.inst, d.locked and Color3.fromRGB(255, 60, 60) or Color3.fromRGB(60, 255, 60) end end
    end
})
SettingsTab:AddToggle({ 
...
local function SafeDoorESP_Patch()
    ...
    local function updateDoorBillboard(inst, text, color)
        ...
        if bb and bb:FindFirstChild("TextLabel") then
            ...
            if color then tl.TextColor3 = color end
        end
    end
...
if DoorESPPatch then
    -- Example usage (optional - remove if you want to integrate manually):
    print("🚪 Door ESP Patch is ready to use!")
    print("   - DoorESPPatch.createBillboard(inst, text, color)")
    print("   - DoorESPPatch.updateBillboard(inst, text, color)")
    print("   - DoorESPPatch.removeBillboard(inst)")
end
